...
FOOD_COLUMNS = tuple(FOOD_CATEGORY_COLUMNS)
BUSINESS_COLUMNS = tuple(chr(column_number) for column_number in range(ord("B"), ord("P") + 1))
VEHICLE_RECIPIENTS = {
    "14 foot truck",
    "14 ft truck",
    "ford van",
...
def calculate_business_signature(
    ...
    business_data = {column: raw_data.get(column) for column in BUSINESS_COLUMNS}
...
def find_row_issues(
    ...
    for column in FOOD_COLUMNS:
        ...
        if value is None:
            issues.append(f"food column {column} contains a nonnumeric value")
    ...
    if not is_blank(recorded_total_raw):
        ...
        elif food_values_are_valid and abs(food_total - recorded_total) > Decimal("0.01"):
            issues.append(
                f"food amount sum {food_total} does not match recorded total {recorded_total}"
            )
        ...
        report_rows.append(
            {
                ...
                "recipient_location": (row.raw_data.get("D") or ""),
                "driver": row.raw_data.get("E") or "",
                "vehicle": row.raw_data.get("F") or "",
                "recorded_total_pounds": (row.raw_data.get("P") or ""),
                "issues": " | ".join(issues),
            }
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Review staged historical delivery rows.")
    parser.add_argument(
        "--batch-id",
        required=True,
        type=uuid.UUID,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("data/reports/historical_import_review.csv"),
